...
| C | referrals | 🟢 | `orchestrator/referrals.py` issues deterministic referral codes, detects/redacts self-redemption, and records `referral_issued` / `referral_redeemed` evidence; covered by referral tests and WhatsApp growth smoke |
| C | WhatsApp follow-up | 🟢 | `whatsapp:follow_up_due` sends a pickup reminder through `whatsapp_send` after checking `square_recent_orders`, appends referral pitch when generated by the scheduler path, and records `channel_outbound` evidence |
| C | upsell logic | 🟢 | `website/lib/upsell.ts` suggests add-ons, extra quantity, or local delivery quote from website/order-assistant intent; `/api/assistant` records `upsell_offered` evidence on accepted order paths |
...
scoring, referrals, upsell logic, and a follow-up-due handler. See
...
4. **Latest outbound evidence rerun** (~30 min) — re-run `scripts/e2e_smoke.sh` with a non-overloaded kitchen seed so accept branch plus WA/IG queued/actual outbound rows land in the newest committed evidence.
